using System.Collections;
using System.Collections.Generic;
using Unity.Collections;
using Unity.VisualScripting;
using UnityEngine;
using UnityEngine.Experimental.Rendering;

public class BreakthroughMove : MonoBehaviour
{
    public virtual List<Vector2> AroundList(BasePieceMoveMark basePieceMoveMark)
    {
            var list = new List<Vector2>();
            int myX = (int)transform.position.x;
            int myY = (int)transform.position.y * -1;
            foreach(var around in basePieceMoveMark.canMove)
            {
                Vector2 Newaround = around + new Vector2(myX,myY);
                list.Add(Newaround);
            }

            return list;
    }
    public List<BasePieceMove>AroundBasePieceMoveList
    {
        get
        {
            var list = new List<BasePieceMove>();
            foreach (var item in AroundList(Board.Boardinstans.basePieceMoveMark))
            {
                BasePieceMove pieceMove = Board.Boardinstans.GetPieceMove((int)item.x,(int)item.y);
                if(pieceMove != null)
                {
                    list.Add(pieceMove);
                }
            }
            return list;
        }
    }

    public bool ExistEnemy
    {
        get
        {
            BasePieceMoveMark pieceMove = Board.Boardinstans.GetPieceMoveMark((int)transform.position.x,(int)transform.position.y);
            if(pieceMove != null)
            {
                if(pieceMove.affiliation != Board.Turn)
                {
                    return true;
                }
                else
                {
                    return false;
                }
            }
            else
            {
                return false;
            }
        }
    }
}